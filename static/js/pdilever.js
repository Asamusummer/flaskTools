function group(a) {
    baseData = a, this.go = function () {
        dataFormat(baseData), document.getElementById("leftCont").innerHTML = renderLeft(baseData, baseDom), chooseParent(), renderRight(baseData), setInterval(function () {
            realSearch()
        }, 100)
    }
}
...
function renderLeft(a, b) {
    ...
    if (b.innerHTML = "", "[object Array]" === Object.prototype.toString.call(a))
        for (c = 0; c < a.length; c++) d = "", e = "icon__round--false", f = "", g = getPeoNum(a[c]), h = "", 0 == a[c].last && (h = '<div class="to__number">（ ' + g + " 权限）</div>"), oldSearch && getParent(a[c].group) && (f = "（" + getParent(a[c].group) + "）"), 1 == a[c].active && (e = "icon__round--true", d = "checked = true"), i = "to__subItem", i = "to__subItem to__show", j = document.createElement("div"), k = '<span class="to__dropdownList" ><i onclick="dropClick(this)"><svg t="1550632829702" class="icon" style="" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1783" xmlns:xlink="http://www.w3.org/1999/xlink" width="100%" height="100%"><defs><style type="text/css"></style></defs><path d="M959.52557 254.29773 511.674589 702.334953 63.824631 254.29773Z" p-id="1784"></path></svg></i></span>', 1 == a[c].last && (k = ""), l = "", l = 1 == a[c].last ? a[c].img || "/static/images/header-icon.png" : a[c].img || "/static/images/icon-folder.svg", m = '<span class="to__prefix"><img src="' + l + '"></img></span>', j.innerHTML = '<div class="to__item " id="group_' + a[c].group + '">' + k + '<span onclick="nodeClick(this)"><input id="input' + a[c].group + '" type="checkbox"  ' + d + '" name="cName"  value="' + a[c].name + '" onclick="checkboxClick(this)" />' + m + '<div class="to__name">' + a[c].name + "</div>" + h + '<div class="to__parentName">' + f + '</div><span class="icon__round ' + e + '"></span></span></div>', b.appendChild(j), n = document.createElement("div"), n.className = i, j.appendChild(n), a[c].children.length > 0 && 1 != a[c].last ? (d = !1, i = "to__subItem", renderLeft(a[c].children, n)) : (d = !1, i = "to__subItem");
...
function renderRight(a) {
    chooseCount = 0, domRight.innerHTML = "", chooseList = [], loopRight(a), document.getElementById("selectNum").innerHTML = chooseCount
}

function loopRight(a) {
    var b, c, d, e;
    for (b = 0; b < a.length; b++) c = a[b], c.children.length > 0 && 1 != c.last ? loopRight(c.children) : 1 != c.last && !oldSearch || 1 != c.active || (d = "", getParent(c.group) && (d = "（" + getParent(c.group) + "）"), chooseCount++, e = document.createElement("div"), e.className = "to__item", e.innerHTML = '<span class="to__prefix"><img src="/static/images/header-icon.png"></img></span><span class="to__name" id="render' + c.group + '" >' + c.name + '</span><div class="to__parentName">' + d + '</div><span class="to__close" onclick="nodeCencel(this)"><i></i></span>', domRight.appendChild(e), c.value && chooseList.push(c.value))
}
...
function nodeClick(a) {
    ...
    i = b.id.replace("input", ""), chooseState(baseData, i), renderRight(baseData), console.log("添加后的数组: "), console.log(chooseList);
