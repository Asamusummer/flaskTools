...
Validator = {
	...
	Mobile : /^((\(\d{2,3}\))|(\d{3}\-))?((1[3-9]\d{9}))$/,
	Url : /^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/,
	Urls : /^https:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/,
	Url2 : /^http:\/\/+[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/,
	...
	Integer : /^[-\+]?\d+$/,
	Double : /^[-\+]?\d+(\.\d+)?$/,
	PositiveDouble : /^[1-9]\d{0,7}(\.\d+)?$/,
	English : /^[A-Za-z]+$/,
	Chinese :  /^[\u0391-\uFFE5|\·]+$/,
	Username : /^[a-z]\w{3,19}$/i,
	User_name : /^\w{4,16}$/i,
	Password : /^((?!\d+$)(?![a-zA-Z]+$)(?!_+$)[a-zA-Z\d_]{7,15})+$/,//密码必须8-16位，数字、字母、下划线，至少三种中的两种组合
		...
		if(this.ErrorMessage.length > 1){
			...
			case 1 :
				...
				if ((this.ErrorItem[1].tagName.toUpperCase() == "INPUT" 
				  ...
				  ||this.ErrorItem[1].multiple) {// Fix select multiple can not get focus problems! Fix by Wu,Yang 2011-03-12
					//donothing
				} else {
...
function showMessage(eMessage){
	eMessage.shift();
	BootstrapDialog.show({
		title:"系统提示",
	    message:eMessage.join("\n"),
